...
Result:
Method 2（累积结果并批量转换为DataFrame）在性能上显著优于Method 1（预分配DataFrame）。
测试结果显示：
1. 执行时间：Method 2比Method 1快约50-60倍（100个数据块：14.2秒 vs 0.28秒）
2. 内存使用：Method 2略高于Method 1（170.61 MB vs 155.25 MB），增加约10%
3. 不同的保存间隔对Method 2的性能影响不大，但大的保存间隔稍微更有效率
4. 随着数据量增加，两种方法的性能差距会更加明显

结论：对于需要处理大量数据块的场景，Method 2（累积结果并定期批量转换）明显更高效。
虽然内存消耗略高，但执行速度的巨大提升使其成为更优的选择。
"""
